...
export default function WalletDetailPane({ cert, onDownload, onBackMobile }) {
  ...
  if (!cert) {
    return (
      <div className="wallet-detail-pane" style={{ textAlign: "center", padding: "4rem 1.5rem", color: GS.muted }}>
        <p style={{ fontSize: "1.1rem", fontWeight: 700, color: GS.ink, marginBottom: "0.5rem" }}>
          Select a certificate to view details
        </p>
        <p style={{ fontSize: "0.85rem", color: GS.muted }}>
          Click any credential row in the master index list on the left to inspect its full cryptographic details.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="wallet-detail-pane">
      {/* Mobile Back Button */}
      {onBackMobile && (
        <button
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "0.75rem", marginBottom: "1.25rem" }}>
            <div>
              <h3 style={{ margin: 0, fontSize: "1.4rem", fontWeight: 800, color: GS.ink }}>{cert.course}</h3>
              <div style={{ fontSize: "0.88rem", color: GS.muted, marginTop: "3px" }}>{cert.university_name || "Issuing University"}</div>
            </div>
          ...
          >
            ...
            <div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                ...
                <button className={`wallet-copy-btn${copiedId ? " copied" : ""}`} onClick={handleCopyId}>
          ...
          {/* Full-size High Resolution Rendered Certificate Graphic */}
          <div
            style={{
              background: "#f8f9fa",
              border: `1.5px solid ${GS.border}`,
          ...
          >
            <div
              style={{
                transform: isVertical ? "scale(0.5)" : "scale(0.55)",
                transformOrigin: "top center",
                marginBottom: isVertical ? "-420px" : "-240px",
              }}
            >
